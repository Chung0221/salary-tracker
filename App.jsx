...
const SalaryTracker = () => {
  ...
  const copyForSheets = () => {
    ...
    tsv += "日期\t上班\t下班\t正常工時\t1.34加班\t1.67加班\t總加班\t薪資\t備註\n";
    
    // 內容 (按日期正序排列，方便 Sheets 查看)
    ...
    alert('已複製報表內容（含總計）！請直接到 Google Sheets 貼上即可。');
      ...
      <div className="max-w-6xl mx-auto space-y-6">
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-6">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
            <div className="space-y-1"><span className="text-xs font-bold text-slate-400">日期</span><input type="date" value={newRecord.date} onChange={e => setNewRecord({...newRecord, date: e.target.value})} className="w-full p-2 bg-slate-50 rounded-lg border border-slate-200 focus:outline-blue-500"/></div>
            <div className="space-y-1"><span className="text-xs font-bold text-slate-400">上班時間</span><input type="time" value={newRecord.checkIn} onChange={e => setNewRecord({...newRecord, checkIn: e.target.value})} className="w-full p-2 bg-slate-50 rounded-lg border border-slate-200 focus:outline-blue-500"/></div>
            <div className="space-y-1"><span className="text-xs font-bold text-slate-400">下班時間</span><input type="time" value={newRecord.checkOut} onChange={e => setNewRecord({...newRecord, checkOut: e.target.value})} className="w-full p-2 bg-slate-50 rounded-lg border border-slate-200 focus:outline-blue-500"/></div>
            <div className="space-y-1"><span className="text-xs font-bold text-slate-400">類別</span><select value={newRecord.note} onChange={e => setNewRecord({...newRecord, note: e.target.value})} className="w-full p-2 bg-slate-50 rounded-lg border border-slate-200 focus:outline-blue-500"><option value="">正常上班</option><option value="病假">病假</option><option value="雙薪">雙薪</option></select></div>
            <div className="flex items-end"><button onClick={addRecord} className="w-full bg-blue-600 text-white py-2 rounded-lg font-bold hover:bg-blue-700 transition shadow-lg shadow-blue-100 active:scale-95">新增紀錄</button></div>
          </div>
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
          <div className="p-4 bg-slate-50 border-b flex justify-between items-center">
            <div className="flex items-center gap-2"><Calendar size={16} className="text-slate-400" /><select value={filterMonth} onChange={e => setFilterMonth(e.target.value)} className="bg-white border border-slate-200 rounded-lg p-1 text-sm font-bold"><option value="all">所有月份</option>{[...new Set(records.map(r => r.date.substring(0, 7)))].map(m => <option key={m} value={m}>{m}</option>)}</select></div>
            ...
            <table className="w-full text-left whitespace-nowrap">
              <thead>
                <tr className="text-[11px] text-slate-400 border-b bg-slate-50/30 uppercase tracking-tighter font-black">
                  <th className="p-4">日期 / 時間</th><th className="p-4">正常</th><th className="p-4 text-center">1.34 / 1.67 加班</th><th className="p-4">總加班</th><th className="p-4">薪資估算</th><th className="p-4 text-right">操作</th>
                </tr>
              </thead>
              <tbody>
                {filteredRecords.map(r => (
                  <tr key={r.id} className="border-b hover:bg-slate-50/80 transition group">
                    <td className="p-4"><div className="text-sm font-bold text-slate-700">{r.date}</div><div className="text-[10px] text-slate-400 flex items-center gap-1 mt-0.5"><Clock size={10}/> {r.checkIn} - {r.checkOut}</div></td>
