...
import { Download, Plus, Trash2, Calendar, Archive, FileSpreadsheet, Table, CheckCircle2, Eye, EyeOff, Copy, Clock } from 'lucide-react';
...
const SalaryTracker = () => {
  ...
  return (
    <div className="min-h-screen bg-slate-50 p-4 md:p-8 pb-40 text-slate-900">
      <div className="max-w-6xl mx-auto space-y-6">
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-6 flex justify-between items-center">
          ...
          <button onClick={() => setShowSettings(!showSettings)} className="px-4 py-2 bg-slate-100 text-slate-600 rounded-lg hover:bg-slate-200 transition font-bold text-sm">
             ⚙️ 系統設定
          </button>
        </div>
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-6">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
            ...
            <div className="space-y-1">
              ...
              <input type="time" value={newRecord.checkIn} onChange={e => setNewRecord({...newRecord, checkIn: e.target.value})} className="w-full p-2 bg-slate-50 rounded-lg border border-slate-200 focus:outline-blue-500"/>
            </div>
            <div className="space-y-1">
              <span className="text-xs font-bold text-slate-400">下班時間</span>
              <input type="time" value={newRecord.checkOut} onChange={e => setNewRecord({...newRecord, checkOut: e.target.value})} className="w-full p-2 bg-slate-50 rounded-lg border border-slate-200 focus:outline-blue-500"/>
            </div>
            <div className="space-y-1">
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
          <div className="p-4 bg-slate-50 border-b flex justify-between items-center">
            <div className="flex items-center gap-2">
              ...
              <select value={filterMonth} onChange={e => setFilterMonth(e.target.value)} className="bg-white border border-slate-200 rounded-lg p-1 text-sm font-bold">
                <option value="all">所有月份</option>
                {[...new Set(records.map(r => r.date.substring(0, 7)))].map(m => <option key={m} value={m}>{m}</option>)}
              </select>
            </div>
            <button onClick={() => setShowExportModal(true)} className="text-emerald-600 flex items-center gap-1 font-bold text-sm hover:text-emerald-700 bg-emerald-50 px-3 py-1.5 rounded-lg transition"><Download size={16}/> 匯出報表</button>
          </div>
          <div className="overflow-x-auto">
            <table className="w-full text-left">
              <thead>
                <tr className="text-xs text-slate-400 border-b bg-slate-50/30 uppercase tracking-tighter font-black">
                  <th className="p-4">日期 / 時間</th>
                  <th className="p-4">正常</th>
                  <th className="p-4 text-center">1.34 / 1.67 加班</th>
                  <th className="p-4">總加班</th>
                  <th className="p-4">薪資估算</th>
              ...
              <tbody>
                  ...
                  <tr key={r.id} className="border-b hover:bg-slate-50/80 transition group">
                    <td className="p-4">
                        ...
                        <div className="text-[10px] text-slate-400 flex items-center gap-1 mt-0.5">
                            <Clock size={10}/> {r.checkIn} - {r.checkOut}
                        </div>
                    </td>
                    <td className="p-4 text-sm text-slate-600 font-medium">{Number(r.regularHours || 0).toFixed(1)}h</td>
                    <td className="p-4 text-center">
                        <div className="flex items-center justify-center gap-2">
                            <span className="text-orange-500 font-bold bg-orange-50 px-1.5 py-0.5 rounded text-xs">{Number(r.overtime1 || 0).toFixed(2)}h</span>
                            <span className="text-orange-500 font-bold bg-orange-50 px-1.5 py-0.5 rounded text-xs">{Number(r.overtime2 || 0).toFixed(2)}h</span>
                        </div>
                    </td>
                    ...
                    <td className="p-4 text-right">
                        <button onClick={() => {setDeleteTarget(r); setShowDeleteModal(true)}} className="p-2 text-slate-200 hover:text-red-500 hover:bg-red-50 rounded-lg transition opacity-0 group-hover:opacity-100">
                            <Trash2 size={16}/>
                        </button>
                    </td>
                  </tr>
                ...
                <tr className="bg-slate-50 font-black text-slate-700 border-t-2 border-slate-200">
                  <td className="p-4 text-sm font-black">當月總結</td>
                  <td className="p-4 text-xs text-slate-400 italic">8h/日</td>
                  <td className="p-4 text-center">
                    <div className="flex items-center justify-center gap-2 text-xs">
                        <span className="text-orange-700">{totals.ot1.toFixed(2)}h</span>
                        <span className="text-slate-300">/</span>
                        <span className="text-orange-700">{totals.ot2.toFixed(2)}h</span>
                    </div>
                  </td>
                  <td className="p-4 text-orange-700 font-black">{totals.otTotal.toFixed(2)}h</td>
                  <td className="p-4 text-emerald-700 font-black text-lg">NT$ {totals.salary.toLocaleString()}</td>
                  <td className="p-4"></td>
                </tr>
              </tbody>
            </table>
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-white/90 backdrop-blur-md border-t border-slate-200 p-6 shadow-[0_-10px_30px_rgba(0,0,0,0.05)] z-40">
        <div className="max-w-6xl mx-auto flex justify-between items-center">
          <div className="flex flex-col gap-1">
            <span className="text-[10px] text-slate-400 font-black uppercase tracking-[0.2em]">預估實領薪資總額</span>
            <div className="flex items-center gap-3">
              <span className="text-3xl font-black text-emerald-600 tracking-tighter">
                {showSalary ? `NT$ ${totals.salary.toLocaleString()}` : 'NT$ ********'}
              </span>
              <button onClick={() => setShowSalary(!showSalary)} className="p-2 bg-slate-100 hover:bg-slate-200 rounded-full transition text-slate-400">
                {showSalary ? <EyeOff size={18}/> : <Eye size={18}/>}
              </button>
            </div>
          </div>
          <div className="flex flex-col items-end gap-1">
             <span className="text-[10px] text-orange-400 font-black uppercase tracking-[0.2em]">總加班時數</span>
             <div className="px-4 py-1.5 bg-orange-600 text-white rounded-full font-black text-sm shadow-lg shadow-orange-100">
                {totals.otTotal.toFixed(2)} <span className="text-[10px] opacity-80 ml-0.5">HR</span>
             </div>
          </div>
        </div>
      </div>
        ...
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center z-50 p-4 animate-in fade-in duration-200">
          <div className="bg-white rounded-3xl p-8 max-w-sm w-full shadow-2xl scale-in-center">
            ...
            <div className="space-y-3">
              <button onClick={copyForSheets} className="w-full py-4 bg-emerald-600 text-white rounded-2xl font-black flex items-center justify-center gap-3 hover:bg-emerald-700 transition shadow-xl shadow-emerald-100 active:scale-95">
        ...
        <div className="fixed inset-0 bg-slate-900/60 flex items-center justify-center z-50 p-4">
          <div className="bg-white p-8 rounded-3xl max-w-xs w-full shadow-2xl">
            ...
            <div className="flex gap-3">
              ...
              }} className="flex-1 py-3 bg-red-500 text-white rounded-xl font-black hover:bg-red-600 transition shadow-lg shadow-red-100">刪除</button>
